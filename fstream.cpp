...
/// Penrillian: Sorted out CREATE issues.
/*static*/ int fstream::om_to_flags (openmode m)
{
    ...
    int inout = m&(in|out);
    int flags = (inout == in) ? O_RDONLY : (inout == out) ? (O_WRONLY|O_CREAT) : O_RDWR;
...
{
	int flags = om_to_flags(mode);
    int nfd = ::open (filename, flags, perms);
    attach (nfd, filename);
}
...
{
    ssize_t brn;
    do { brn = ::read (m_fd, (char*)p, n); } while (brn < 0 && errno == EINTR);
    if (brn > 0)
	return (brn);
...
off_t fstream::write (const void* p, off_t n)
	...
	while (btw) {
		...
		ssize_t bwn = ::write (m_fd, (char*)advance(p,bw), btw);
		if (bwn > 0)
			btw -= bwn;
		else if (!bwn) {
...
{
    int rv = ::ioctl (m_fd, request, (void*)argument);
    if (rv < 0)
...
{
    int rv = 0;//::fcntl (m_fd, request, argument); // Symbian OS doesn't support fcntl!
    if (rv < 0)
	set_and_throw (failbit, rname);
    return (rv);
}
