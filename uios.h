...
public:
    ...
    enum openmode_bits {
	in	= (1 << 0), ///open for input
	out	= (1 << 1), ///open for output
	app	= (1 << 2), ///seek to end before each write
	ate	= (1 << 3), ///open and seek to end immediately after opening     
	binary	= (1 << 4), /// perform input and output in binary mode (as opposed to text mode)
	trunc	= (1 << 5), ///truncate an existing stream when opening
	#ifndef DOXYGEN_SHOULD_SKIP_THIS
	nonblock= (1 << 6),
	nocreate= (1 << 7), // Not in ANSI standard.
	noctty	= (1 << 8),
	nombits	= 9
	#endif
    };
